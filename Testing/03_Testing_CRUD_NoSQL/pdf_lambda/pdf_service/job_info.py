...
def build_job_info(jobcard, usable_width, meta_style):
    ...
    left_table = Table([
        [Paragraph(
            f"<b>Jobcard No:</b> {jobcard.get('jobcardNumber', '')}", meta_style)],
        [Paragraph(
            f"<b>Requested By:</b> {jobcard.get('requested_by', '')}", meta_style)],
        [Paragraph(f"<b>Date Requested:</b> {jobCreated}", meta_style)],
        [Paragraph(
            f"<b>Asset Description:</b> {jobcard['equipment']}", meta_style)],
        [Paragraph(
            f"<b>Asset ID:</b> {jobcard.get('assetID', '')}", meta_style)],
        [Paragraph(f"<b>Location:</b> {jobcard['location']}", meta_style)],
    ], colWidths=[usable_width * 0.48])

    right_table = Table([
        [Paragraph(
            f"<b>Work Order No:</b> {jobcard.get('work_order_number', '')}", meta_style)],
        [Paragraph(
            f"<b>Actioned By:</b> {jobcard.get('actioned_by', '')}", meta_style)],
