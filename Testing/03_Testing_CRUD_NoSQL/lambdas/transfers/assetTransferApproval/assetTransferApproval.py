...
def get_branch_manager(location):
    ...
    for user in items:
        if user.get("poition", "").lower() == "supervisor":
            return user
...
def lambda_handler(event, context):
    ...
    for record in event.get("Records", []):
        ...
        location_from = new_image["locationFrom"]
        location_to = new_image["locationTo"]

        approved_date = new_image["approvedDate"]

        recipient = get_branch_manager(location_to)
...
# ---------------------------------------------------------------------------- #
        ...
        requestor_notification = {
            ...
            "message": (
                f"Your transfer request for asset {asset_id} "
                f"from {location_from} to {location_to} has been approved."
            ),
            "channels": [
                "IN_APP",
                "EMAIL"
    ...
    if recipient:
        ...
        recipient_notification = {
            ...
            "assetId": asset_id,
            "message": (
                f"Asset {asset_id} has been approved for transfer "
                f"to {location_to} and will arrive from {location_from}."
            ),
            "channels": [
                "IN_APP",
                "EMAIL"
