...
def lambda_handler(event, context):
    """
    Set an asset transfer in-transit.

    The lambda is triggered with path: `/transfers/0bdc65f2-c66a-4f9b-8a59-8c78845131d0/in-transit`

    This Lambda function updates an existing asset transfer request that has been approved to "in-transit". The frontend
            ...
            body:
                {
                    ...
                    "transportName" : "fabian petersen",
                    "transportCost" : "345",
                    "transportDate" : "2026-07-08T09:26",
                    "transportNotes" : "asset will be collected tomorrow morning.",
                    "images" : [],
                    "invoices" : []
                }
    ...
    Response Codes:
        200 - Transfer in-transit successfull.
        400 - Invalid request.
        409 - Transfer is no longer in the approved state.
    ...
    try:
        ...
        if status != "in-transit":
            return _response(
                400,
                {"message": "Only the status 'in-transit' is supported."}
            )
        ...
        if normalize_string(transfer_item.get("status")) != "approved":
        ...
        if error_code == "ConditionalCheckFailedException":
            return _response(409, {"message": "Transfer is no longer in the approved state."})
